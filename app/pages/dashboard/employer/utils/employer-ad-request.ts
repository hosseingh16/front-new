...
function resolveFirstLookupLabel(
  ...
  for (const value of normalized) {
    const match = options.find(
      (item) => String(item.value) === value || item.label.trim() === value,
    );
...
export function getAdRequestSalary(request: EmployerAdRequest) {
  ...
  if (typeof salary === "string" && salary.trim() === "حقوق پایه") {
...
export function getAdRequestAge(request: EmployerAdRequest) {
  ...
  const age =
    birthYear > 1300 ? 1405 - birthYear : new Date().getFullYear() - birthYear;
