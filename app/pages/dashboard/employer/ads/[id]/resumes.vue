<template>
  <div>
    <nav class="mb-4 hidden text-sm text-text-passive md:block">
      ...
      <NuxtLink to="/dashboard/employer/ads" class="transition-colors hover:text-primary-500">
        آگهی‌های من
      </NuxtLink>
      <span class="mx-2">/</span>
    ...
    <template v-if="!initialized">
      <div class="h-8 w-48 animate-pulse rounded bg-surface-200" />
      <div class="mt-6 space-y-4">
        <div v-for="n in 3" :key="`skeleton-${n}`" class="h-48 animate-pulse rounded-2xl bg-surface-200" />
      </div>
    </template>

    <p v-else-if="!adId" class="py-12 text-center text-sm text-error">
    ...
    <template v-else>
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex min-w-0 items-center gap-2">
          <h1 class="truncate font-yb-bold text-lg text-text-tertiary md:text-xl">
            {{ adTitle }}
          </h1>
          <span class="shrink-0 rounded-full bg-surface-100 px-3 py-1 text-xs font-semibold text-text-passive">
      ...
      <Teleport to="body">
        ...
        >
          <div class="absolute inset-0 bg-black/40" @click="closeMobileTabModal" />
          <div class="absolute inset-x-0 bottom-0 flex flex-col rounded-t-2xl bg-white">
            ...
            <div class="px-4 py-5">
              <div class="flex items-center gap-2 rounded-lg border border-gray-default bg-[#F6F8FE] p-1">
                <button
                  v-for="tab in tabs"
                  :key="tab.id"
                  type="button"
      ...
      <div class="mt-4 grid items-start gap-4 md:grid-cols-7">
        <AdRequestFiltersSidebar v-model="requestFilters" class="md:col-span-2" />

        <div id="ad-requests-results" class="md:col-span-5">
...
const adTitle = computed(() => {
  const fromAds = adGroups.value.total.find((ad) => ad.id === adId.value)?.title
...
const filteredRequests = computed(() => {
  ...
  return requests.value.filter((request) => {
    ...
    const personalTitle = request.user?.personal?.job_title?.toLowerCase() ?? ''
