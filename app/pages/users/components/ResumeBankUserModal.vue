    ...
    >
      <div class="relative w-full max-w-[1580px]" @click.stop>
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="resume-bank-modal-title"
        >
          <header
...
import { resolveResumeAttachment } from "~/pages/users/utils/user-resume";

const props = defineProps<{
  open: boolean;
  user: ResumeBankUser | null;
  users?: ResumeBankUser[];
}>();

const emit = defineEmits<{
  "update:open": [value: boolean];
  "update:user": [value: ResumeBankUser];
  bookmarkChange: [userId: number, bookmarked: boolean];
}>();

const { $toast } = useNuxtApp();

const userId = computed(() => props.user?.id ?? 0);
const isBookmarked = computed(() => Boolean(props.user?.is_bookmarked));
const { user, loading: userLoading, error: userError } = useUser(userId);

const currentIndex = computed(() => {
  if (!props.user) return -1;
  return (props.users ?? []).findIndex((item) => item.id === props.user?.id);
});
...
const canGoNext = computed(
  () =>
    ...
    currentIndex.value < (props.users?.length ?? 0) - 1,
);
...
function goPrev() {
  if (!canGoPrev.value || !props.users?.length) return;
  const prevUser = props.users[currentIndex.value - 1];
  if (prevUser) emit("update:user", prevUser);
}

function goNext() {
  if (!canGoNext.value || !props.users?.length) return;
  const nextUser = props.users[currentIndex.value + 1];
  if (nextUser) emit("update:user", nextUser);
}

function onBookmarkChange(value: boolean) {
  if (!props.user) return;
  props.user.is_bookmarked = value;
  emit("bookmarkChange", props.user.id, value);
}
