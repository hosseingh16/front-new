<template>
  <div class="bg-surface-50 pb-12">
    <div class="custom-pad mx-auto pt-6">
      ...
      <template v-else-if="loading && !company">
      ...
      <template v-else-if="company">
        <div class="mt-6 flex items-center gap-6 border-b border-surface-200">
          <button
            type="button"
            class="border-b-2 pb-2 text-sm font-semibold transition-colors cursor-pointer"
...
const companyInitial = computed(() => company.value?.name?.charAt(0) ?? 'ش')

const locationLabel = computed(() => {
  const province = company.value?.province_name
  const city = company.value?.city_name
...
const showContactInfo = computed(
  () => (company.value?.tel && isTelPublic.value) || company.value?.address,
)
