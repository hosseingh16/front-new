    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          ...
          >
            <ResumeBankFilterSectionContent :section-index="activeSection" />
          </div>
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto px-4">
          ...
          <div v-else class="py-4 text-sm text-text-tertiary">
            <ResumeBankFilterSectionContent
              :section-index="detailSectionIndex!"
            />
          </div>
        </div>
...
import { RESUME_BANK_FILTER_SECTIONS } from "~/types/resume-bank";

const filters = (() => {
  const ctx = inject(RESUME_BANK_FILTERS_INJECTION_KEY);
  if (!ctx) {
    throw new Error(
      "ResumeBankFiltersMobile must be used within a resume bank filters provider",
    );
  }
  return ctx;
})();
...
function sectionTitle(index: number) {
  return RESUME_BANK_FILTER_SECTIONS[index]?.title ?? "";
}
  ...
  switch (sectionIndex) {
    case 0:
      return (
        ...
        filters.selectedJobTitles.length > 0
      );
    case 1:
      return filters.selectedSkills.length > 0;
    case 2:
      return (
        filters.selectedJobStatuses.length > 0 ||
        filters.selectedExperience.length > 0
      );
    case 3:
      return filters.selectedSalaries.length > 0;
    case 4:
      return filters.selectedEducation.length > 0;
    case 5:
...
function chipLabel(sectionIndex: number, title: string) {
  ...
  if (sectionIndex === 0 && filters.titleSearch.trim()) {
    return filters.titleSearch.trim();
  }
...
function sectionOptions(sectionIndex: number) {
  switch (sectionIndex) {
    case 0:
      return filters.filteredJobTitles;
    case 1:
      return filters.filteredSkills;
    case 2:
      return [...filters.jobStatusOptions, ...filters.experienceLevels];
    case 3:
      return filters.salaryRanges;
    case 4:
      return filters.educationLevels;
    case 5:
      return filters.filteredProvinces;
    case 6:
      return filters.genderOptions;
    default:
      return [];
  }
}
...
function sectionSelectedValues(sectionIndex: number) {
  switch (sectionIndex) {
    case 0:
      return filters.selectedJobTitles;
    case 1:
      return filters.selectedSkills;
    case 2:
      return [...filters.selectedJobStatuses, ...filters.selectedExperience];
    case 3:
      return filters.selectedSalaries;
    case 4:
      return filters.selectedEducation;
    case 5:
      return filters.selectedProvinces;
    case 6:
      return filters.selectedGenderValues;
    default:
      return [];
  }
}
