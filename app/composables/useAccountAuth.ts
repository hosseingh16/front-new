...
export function useAccountAuth() {
  ...
  const selectedRole = useState<AccountRole | null>(
  ...
  async function requestOtpViaVoice() {
    ...
    loading.value = true;
    try {
      const res = await client<RequestOtpVoiceResponse>(
        "/api/request-otp-voice",
        {
          method: "POST",
  ...
  async function loginWithMobile(phone?: string, redirect = true) {
    ...
    try {
      if (!isAuthenticated.value) {
        // Sanctum login already POSTs /api/login and GETs /api/user.
